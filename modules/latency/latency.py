...
DEFAULTS = {"hosts": ["1.1.1.1", "8.8.8.8"], "timeout_ms": 2000} if "latency"=="latency" else {"target":"1.1.1.1", "timeout_ms":2000}
CONFIG = _load_config("latency", DEFAULTS)
...
def enable():
    global _ENABLED, _THREAD; _ENABLED = True; _ensure_dirs("latency")
    if CONFIG.get("autostart") and _THREAD is None:
        _STOP.clear(); _THREAD = threading.Thread(target=_worker_loop, args=("latency", _job, [".host"]), daemon=True); _THREAD.start()
...
def health():
    if "latency"=="latency":
        ...
        for h in hosts:
            ...
            if ok: return {"ok": True, "module":"latency", "via":"tcp:443", "host":h, "ms": round(ms,2)}
        return {"ok": False, "module":"latency", "error":"no host reachable"}
    else:
        ok, ms = _tcp(CONFIG.get("target") or "1.1.1.1", 443, (CONFIG.get("timeout_ms") or 2000)/1000.0)
        return {"ok": ok, "module":"latency", "ms": round(ms,2) if ms>=0 else None}

try:
    from module_api import register_simple_module  # type: ignore
    register_simple_module("latency", configure, enable, health)
