...
CONFIG = _load_config("mode_shadow", DEFAULTS)

def configure(cfg): CONFIG.update(cfg or {}); return {"ok": True, "applied": CONFIG}
def enable():
    global _ENABLED; _ENABLED = bool(CONFIG.get("enabled", True)); _ensure_dirs("mode_shadow"); return {"ok": True, "enabled": _ENABLED}
def health(): return {"ok": True, "module":"mode_shadow", "enabled": CONFIG.get("enabled", False)}

try:
    from module_api import register_simple_module  # type: ignore
    register_simple_module("mode_shadow", configure, enable, health)
