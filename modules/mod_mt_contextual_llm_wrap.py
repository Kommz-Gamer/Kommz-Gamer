# Auto-generated wrapper (FIXED) for mt_contextual_llm
# - No dependency on module_api
# - Avoids blueprint collisions by checking existing routes first
# - Registers /api/mod/mt_contextual_llm/health ONLY if missing
# - Always exposes /api/mod/mt_contextual_llm/wrap/health for debugging

import os, importlib.util
from flask import Blueprint, jsonify

MOD_NAME = "mt_contextual_llm"
BP_NAME  = "wrap_" + MOD_NAME
URL_BASE = "/api/mod/" + MOD_NAME

def _try_import_underlying():
    here = os.path.dirname(__file__)
    target = os.path.join(here, f"mt_contextual_llm.py")
