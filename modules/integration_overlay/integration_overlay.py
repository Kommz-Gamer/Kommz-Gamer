# -*- coding: utf-8 -*-
"""
integration_overlay — ULTRA
- Validates overlay file existence; no OS overlay manipulations for safety.
- Queue: .cmd files (line = "show <path>" or "hide") -> state.txt
"""
...
DEFAULTS = {"enabled": True, "opacity":0.85, "position":"top", "overlay_path":"", "autostart": False}
CONFIG = _load_config("integration_overlay", DEFAULTS)

def _job(path: str):
    cmd = open(path, "r", encoding="utf-8").read().strip()
    with open(os.path.join(_queue_dir("integration_overlay"), "state.txt"), "w", encoding="utf-8") as f:
        f.write(cmd)
...
def enable():
    ...
    _ENABLED = True; _ensure_dirs("integration_overlay")
    if CONFIG.get("autostart") and _THREAD is None:
        _STOP.clear(); _THREAD = threading.Thread(target=_worker_loop, args=("integration_overlay", _job, [".cmd"]), daemon=True); _THREAD.start()
    return {"ok": True, "enabled": True}

def health():
    path = CONFIG.get("overlay_path")
    if path: return {"ok": bool(os.path.exists(path)), "module":"integration_overlay", "path_exists": os.path.exists(path)}
    return {"ok": True, "module":"integration_overlay"}

try:
    from module_api import register_simple_module  # type: ignore
    register_simple_module("integration_overlay", configure, enable, health)
