...
DEFAULTS = {"source": "auto", "target": "en", "autostart": False, "auth_key":"", "tier":"free", "formality":"default"}
CONFIG = _load_config("mt_deepl", DEFAULTS)

def _translate(text: str) -> str:
    key = CONFIG.get("auth_key")
    if not key: raise RuntimeError("missing auth_key")
    url = "https://api-free.deepl.com/v2/translate" if CONFIG.get("tier","free")=="free" else "https://api.deepl.com/v2/translate"
    data = {"auth_key": key, "text": text, "target_lang": CONFIG.get("target","EN").upper()}
    if CONFIG.get("source") and CONFIG["source"]!="auto": data["source_lang"]=CONFIG["source"].upper()
...
def _job(path: str):
    ...
    out = _translate(txt)
    with open(path + ".mt_deepl.txt", "w", encoding="utf-8") as f:
        f.write(out)

def configure(cfg): CONFIG.update(cfg or {}); return {"ok": True, "applied": CONFIG}
def enable():
    global _ENABLED, _THREAD
    _ENABLED = True; _ensure_dirs("mt_deepl")
    if CONFIG.get("autostart") and _THREAD is None:
        _STOP.clear(); _THREAD = threading.Thread(target=_worker_loop, args=("mt_deepl", _job, [".txt"]), daemon=True); _THREAD.start()
    return {"ok": True, "enabled": True}
def health(): return {"ok": bool(CONFIG.get("auth_key")), "module":"mt_deepl"}

try:
    from module_api import register_simple_module  # type: ignore
    register_simple_module("mt_deepl", configure, enable, health)
