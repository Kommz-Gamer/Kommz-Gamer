...
DEFAULTS = {}
CONFIG = _load_config("server_info", DEFAULTS)
_START = time.time()

def configure(cfg): return {"ok": True, "applied": CONFIG}
def enable(): 
    global _ENABLED; _ENABLED = True; _ensure_dirs("server_info"); return {"ok": True, "enabled": True}
def health():
    info = {"hostname": socket.gethostname(), "platform": platform.platform(), "python": platform.python_version(), "uptime_s": int(time.time()-_START)}
    try:
        import psutil  # type: ignore
        info["cpu"] = psutil.cpu_percent(interval=0.05)
    except Exception: pass
    return {"ok": True, "module":"server_info", "info": info}

try:
    from module_api import register_simple_module  # type: ignore
    register_simple_module("server_info", configure, enable, health)
except Exception: pass
