...
Config sources (merged left→right):
  ...
  2) File: ./config/mt_contextual_llm.json
  3) Env:  VTP_MT_CONTEXTUAL_LLM_* (JSON for complex fields)
"""
...
CONFIG = _load_config('mt_contextual_llm', DEFAULTS, 'MT_CONTEXTUAL_LLM')

def configure(cfg: Dict[str, Any]) -> Dict[str, Any]:
    global CONFIG; CONFIG = _merge(CONFIG, cfg or {}); return {"ok": True, "applied": CONFIG}
def enable() -> Dict[str, Any]:
    global _ENABLED; _ENABLED = True; return {"ok": True, "enabled": True}
def health() -> Dict[str, Any]:
    return {"ok": True, "module": "mt_contextual_llm", "provider": CONFIG.get("provider")}

try:
    from module_api import register_simple_module  # type: ignore
    register_simple_module("mt_contextual_llm", configure, enable, health)
except Exception:
    pass
