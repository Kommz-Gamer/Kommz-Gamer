# Auto-generated wrapper (FIXED) for integration_overlay
# - No dependency on module_api
# - Avoids blueprint collisions by checking existing routes first
# - Registers /api/mod/integration_overlay/health ONLY if missing
# - Always exposes /api/mod/integration_overlay/wrap/health for debugging

import os, importlib.util
from flask import Blueprint, jsonify

MOD_NAME = "integration_overlay"
BP_NAME  = "wrap_" + MOD_NAME
URL_BASE = "/api/mod/" + MOD_NAME

def _try_import_underlying():
    here = os.path.dirname(__file__)
    target = os.path.join(here, f"integration_overlay.py")
